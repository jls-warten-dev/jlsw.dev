...
(() => {
 ...
 const I18N = {
 es: {
 nav: { proyectos: 'Proyectos', stack: 'Stack', cert: 'Certificaciones', sobre: 'Sobre mí', contacto: 'Contacto' },
 ...
 en: {
 nav: { proyectos: 'Projects', stack: 'Stack', cert: 'Certifications', sobre: 'About me', contacto: 'Contact' },
...
if (certDetails) {
 const syncCert = () => { if (location.hash === '#certificaciones') certDetails.open = true; };
 syncCert();
 window.addEventListener('hashchange', syncCert);
 document.querySelectorAll('a[href="#certificaciones"]').forEach((a) => {
   a.addEventListener('click', () => { certDetails.open = true; });
 });
}
...
/* ── Mobile nav (burger) ────────────────────── */
 ...
 /* ── Post language switch (ES/EN dentro del mismo artículo) ── */
 const postLangs = document.querySelectorAll('.lang-link[data-lang]');
 if (postLangs.length) {
 ...
 const setPostLang = (L) => {
   ...
   const t = document.querySelector(`[data-lang-title="${L}"]`);
